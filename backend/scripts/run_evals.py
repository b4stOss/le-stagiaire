...
def aggregate(records: list[dict]) -> dict:
    def rate(items: list[dict], key: str) -> dict:
        ...
        return {"passed": passed, "total": len(graded), "pct": round(100 * passed / len(graded)) if graded else None}
...
def main() -> None:
    ...
    for q in golden:
        ...
        print(f"    {status}" + (f" ({', '.join(extras)})" if extras else ""), flush=True)
        records.append(record)

    summary = aggregate(records)
    RESULTS_FILE.parent.mkdir(parents=True, exist_ok=True)
    RESULTS_FILE.write_text(
        json.dumps({"run_at": time.strftime("%Y-%m-%d %H:%M"), "summary": summary, "questions": records},
                   ensure_ascii=False, indent=2)
    )

    print("\n=== Summary ===")
    for name, s in [("overall", summary["overall"])] + list(summary["by_category"].items()):
